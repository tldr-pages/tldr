...
`acme.sh --issue --dns {{gnd_gd}} --domain {{example}}`

- Geef een wildcardcertificaat uit (aangegeven met een asterisk) met behulp van een automatische DNS API-modus:

`acme.sh --issue --dns {{dns_namesilo}} --domain {{example}} --domain {{*.example}}`

- Geef een certificaat uit met behulp van een DNS-aliasmodus:

`acme.sh --issue --dns {{dns_cf}} --domain {{example}} --challenge-alias {{alias-voor-voorbeeld-validatie.com}}`
...
`acme.sh --issue --dns {{dns_namecheap}} --domain {{example}} --dnssleep {{300}}`

- Geef een certificaat uit met behulp van een handmatige DNS-modus:

`acme.sh --issue --dns --domain {{example}} --yes-I-know-dns-manual-mode-enough-go-ahead-please`
