...
def main():
    ...
    group.add_argument(
        ...
        dest="clip",
        default=False,
        help="Uses what you copied as URL",
    )
    group.add_argument(
        "-m",
        "--man",
        ...
        dest="man",
        default=False,
        help="Automatically add the manpage for the package as the URL",
    )
    parser.add_argument(
        "-p",
        "--page",
    ...
    target_paths.sort()
    if args.man:
        page_name = os.path.basename(args.page).rstrip(".md")
        link = manurl + page_name
    elif args.clip:
        # You only need those two modules if you use the clipboard switch
        from validators import url as ucheck
        from pyperclip import paste

        link = paste()
        if not ucheck(link):
            raise ValueError(f"{link}\x1b[31m is not a valid URL\x1b[m")
    else:
        link = args.link
