...
- Mostra um grafo de todos os commits, tags e branches em todo o repositório:
...
`git log -i --grep {{cadeia_de_caracteres_de_pesquisa}}`

- Mostra os últimos N commits de um determinado autor:

`git log -n {{number}} --author={{autor}}`

- Mostra commits entre duas datas(aaaa-mm-dd):
