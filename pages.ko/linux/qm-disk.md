...
`qm {{[di|disk]}} {{[resi|resize]}} {{100}} {{디스크_이름}} +{{n}}G`

- 가상 디스크 이동:

`qm {{[di|disk]}} {{[m|move]}} {{100}} {{대상}} {{색인}}`

- 이전 가상 디스크 복사본 삭제:

`qm {{[di|disk]}} {{[m|move]}} --delete {{100}} {{대상}} {{색인}}`

- 특정 스토리지 이름을 사용하여 VMDK/`.qcow2`/raw 디스크 이미지 가져오기:

`qm {{[di|disk]}} {{[i|import]}} {{100}} {{경로/대상/디스크}} {{저장소_이름}} --format {{qcow2|raw|vmdk}}`
