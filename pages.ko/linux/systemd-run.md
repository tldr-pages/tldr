...
`sudo systemd-run {{[-u|--unit]}} {{이름}} --description {{문자열}} {{명령어}} {{인수1 인수2 ...}}`

- 종료 후 정리되지 않는 일시적 서비스와 사용자 정의 환경 변수를 사용하여 시작:

`sudo systemd-run {{[-r|--remain-after-exit]}} --set-env={{이름}}={{값}} {{명령어}} {{인수1 인수2 ...}}`
...
`systemd-run {{[-r|--remain-after-exit]}} --pty {{명령어}}`

- 프로세스의 속성 (예: CPUQuota, MemoryMax)을 설정하고 종료될 때까지 대기:

`systemd-run {{[-p|--property]}} MemoryMax={{메모리_바이트}} {{[-p|--property]}} CPUQuota={{cpu_시간_비율}}% --wait {{명령어}}`

- 셸 파이프라인에서 프로그램 사용:

`{{명령어1}} | systemd-run {{[-P|--pipe]}} {{명령어2}} | {{명령어3}}`
