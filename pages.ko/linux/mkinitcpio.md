...
`sudo mkinitcpio {{[-c|--config]}} {{경로/대상/mkinitcpio.conf}} {{[-g|--generate]}} {{경로/대상/initramfs.img}}`

- 현재 실행 중인 커널이 아닌 다른 커널에 대한 initramfs 이미지 생성 (설치된 커널 릴리스는 `/usr/lib/modules/`에 있음):

`sudo mkinitcpio {{[-k|--kernel]}} {{커널_버전}} {{[-g|--generate]}} {{경로/대상/initramfs.img}}`
