...
`qm cloud-init {{vm_id}} -user={{사용자}} -password={{비밀번호}}`

- 특정 사용자에 대한 cloudinit 설정 구성 및 특정 SSH 키로 사용자 비밀번호 설정:

`qm cloud-init {{vm_id}} -user={{사용자}} -password={{비밀번호}} -sshkey={{ssh_키}}`

- 특정 가상 머신의 호스트네임 설정:

`qm cloud-init {{vm_id}} -hostname={{호스트네임}}`
