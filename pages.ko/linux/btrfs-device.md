...
`sudo btrfs {{[d|device]}} {{[a|add]}} {{경로/대상/블록_장치1 경로/대상/블록_장치2 ...}} {{경로/대상/btrfs_파일_시스템}}`

- btrfs 파일 시스템에서 장치 제거:

`sudo btrfs {{[d|device]}} {{[rem|remove]}} {{경로/대상/장치|장치_ID}} [{{...}}]`

- 오류 통계 표시:

`sudo btrfs {{[d|device]}} {{[st|stats]}} {{경로/대상/btrfs_파일_시스템}}`
...
`sudo btrfs {{[d|device]}} {{[u|usage]}} {{경로/대상/btrfs_파일_시스템}}`
