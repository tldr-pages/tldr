...
`indent {{[-linux|--linux-style]}} {{경로/대상/소스.c 경로/대상/또다른_소스.c ...}}`

- GNU 스타일에 따라 C/C++ 소스 형식을 지정하고, 들여쓰기된 버전을 다른 파일에 저장:

`indent {{[-gnu|--gnu-style]}} {{경로/대상/소스.c}} -o {{경로/대상/들여쓰기된_소스.c}}`

- Kernighan & Ritchie (K&R) 스타일에 따라 C/C++ 소스 형식을 지정하고, 탭이 없으며, 들여쓰기당 공백 3개, 줄바꿈은 120자:

`indent {{[-kr|--k-and-r-style]}} {{[-il|--indent-level]}}3 {{[-nut|--no-tabs]}} {{[-l|--line-length]}}120 {{경로/대상/소스.c}} -o {{경로/대상/들여쓰기된_소스.c}}`
