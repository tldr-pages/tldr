...
`fastd {{[-c|--config]}} {{경로/대상/fastd.conf}}`

- MTU 1400으로 레이어 3 VPN을 시작하고 파일에서 나머지 구성 매개변수를 로드:

`fastd {{[-m|--mode]}} {{tap}} {{[-M|--mtu]}} {{1400}} {{[-c|--config]}} {{경로/대상/fastd.conf}}`

- 구성 파일의 유효성을 검증:

`fastd --verify-config {{[-c|--config]}} {{경로/대상/fastd.conf}}`
...
`fastd --show-key {{[-c|--config]}} {{경로/대상/fastd.conf}}`
