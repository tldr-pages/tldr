...
`repren {{[-n|--dry-run]}} --rename --literal --from '{{찾을_문자열}}' --to '{{교체할_문자열}}' {{*.png}}`

- JPEG 파일이 있는 디렉토리에서 정규 표현식을 사용하여 이름을 바꾸는 예비 실행:

`repren --rename {{[-n|--dry-run]}} --from '{{정규_표현식}}' --to '{{교체할_문자열}}' {{*.jpg}} {{*.jpeg}}`
...
`repren {{[-p|--patterns]}} {{경로/대상/패턴_파일.ext}} --full {{*.txt}}`

- 대소문자를 구분하지 않고 이름 바꾸기:

`repren --rename {{[-i|--insensitive]}} {{[-p|--patterns]}} {{경로/대상/패턴_파일.ext}} *`
