...
`rlwrap {{[-r|--remember]}} {{명령}}`

- 프롬프트에 ANSI 색상 코드가 포함된 경우 더 나은 프롬프트 완성:

`rlwrap {{[-A|--ansi-colour-aware]}} {{명령}}`

- 파일 이름 완성 활성화 (대소문자 구분):

`rlwrap {{[-c|--complete-filenames]}} {{명령}}`

- 색상이 있는 프롬프트 추가, 색상 이름 또는 ANSI 색상 사양 사용. 대문자 색상 이름은 굵게 스타일링:

`rlwrap {{[-p|--prompt-colour=]}}{{black|red|green|yellow|blue|cyan|purple|white|colour_spec}} {{명령}}`
