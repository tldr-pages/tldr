...
`inkscape {{경로/대상/파일명.svg}} {{[-o|--export-filename]}} {{경로/대상/파일명.png}}`

- SVG 파일을 600x400 픽셀의 비트맵으로 내보내기 (가로와 세로 사이의 비율 왜곡이 발생할 수 있음):

`inkscape {{경로/대상/파일명.svg}} {{[-o|--export-filename]}} {{경로/대상/파일명.png}} {{[-w|--export-width]}} 600 {{[-h|--export-height]}} 400`

- SVG 파일의 그림(모든 객체의 경계가 있는 상자)을 비트맵으로 내보냄:

`inkscape {{경로/대상/파일명.svg}} {{[-o|--export-filename]}} {{경로/대상/파일명.png}} {{[-D|--export-area-drawing]}}`

- 해당 ID가 지정된 단일 객체를 비트맵으로 내보냄:

`inkscape {{경로/대상/파일명.svg}} {{[-i|--export-id]}} {{id}} {{[-o|--export-filename]}} {{object.png}}`

- SVG 문서를 PDF로 내보내고 모든 텍스트를 경로로 변경:

`inkscape {{경로/대상/파일명.svg}} {{[-o|--export-filename]}} {{경로/대상/파일명.pdf}} {{[-T|--export-text-to-path]}}`
