...
`naabu {{[-p|-port]}} - -host {{호스트}}`

- 원격 호스트의 상위 1000개 포트 스캔:

`naabu {{[-tp|-top-ports]}} 1000 -host {{호스트}}`

- 원격 호스트의 TCP 포트 80, 443 및 UDP 포트 53 스캔:

`naabu {{[-p|-port]}} 80,443,u:53 -host {{호스트}}`

- 원격 호스트가 사용하는 CDN 유형 표시 (있는 경우):

`naabu {{[-p|-port]}} 80,443 -cdn -host {{호스트}}`

- 추가 기능을 위해 `naabu`에서 `nmap` 실행 (`nmap`이 설치되어 있어야 함):

`sudo naabu {{[-v|-verbose]}} -host {{호스트}} -nmap-cli 'nmap {{-v -T5 -sC}}'`
