...
`tr < {{경로/대상/파일}} '{{abcd}}' '{{jkmn}}'`

- 입력에서 지정된 문자 집합의 모든 발생 삭제:

`tr < {{경로/대상/파일}} {{[-d|--delete]}} '{{입력_문자들}}'`

- 연속된 같은 문자를 하나의 문자로 압축:

`tr < {{경로/대상/파일}} {{[-s|--squeeze-repeats]}} '{{입력_문자들}}'`

- 파일 내용을 대문자로 변환:

`tr < {{경로/대상/파일}} "[:lower:]" "[:upper:]"`

- 파일에서 출력 가능한 문자가 아닌 문자 제거:

`tr < {{경로/대상/파일}} {{[-cd|--complement --delete]}} "[:print:]"`
