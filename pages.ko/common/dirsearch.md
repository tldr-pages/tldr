...
`dirsearch {{[-l|--url-list]}} {{경로/대상/url-list.txt}} {{[-e|--extensions]}} {{php}}`

- 공통 확장자를 사용하여 사용자 정의 경로를 웹 서버에서 검색:

`dirsearch {{[-u|--url]}} {{url}} --extensions-list {{[-w|--wordlists]}} {{경로/대상/url-paths.txt}}`

- 쿠키를 사용하여 웹 서버를 검색:

`dirsearch {{[-u|--url]}} {{url}} {{[-e|--extensions]}} {{php}} --cookie {{쿠키}}`
...
`dirsearch {{[-u|--url]}} {{url}} {{[-e|--extensions]}} {{php}} --json-report {{경로/대상/리포트.json}}`
