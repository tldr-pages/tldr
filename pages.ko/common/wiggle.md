...
`wiggle {{경로/대상/나의_패치.patch}} {{[-o|--output]}} {{경로/대상/출력_파일.txt}}`

- `file.rej`에서 적용되지 않은 변경 사항을 가져와서 파일에 병합:

`wiggle {{[-r|--replace]}} {{경로/대상/파일}} {{경로/대상/파일.rej}}`

- 패치 또는 병합 파일의 한 브랜치 [추출]:

`wiggle {{[-x|--extract]}} {{경로/대상/나의_패치.patch}}`

- 패치를 적용하고 비교된 단어를 [출력] 파일에 저장:

`wiggle {{[-w|--words]}} {{경로/대상/나의_단어_패치.patch}} {{[-o|--output]}} {{경로/대상/단어_패치된_코드.c}}`
