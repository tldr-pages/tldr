...
- 결과 이미지가 지정된 크기를 갖도록 이미지 확장:

`pnmscalefixed -width {{너비}} -height {{높이}} {{경로/대상/입력.pnm}} > {{경로/대상/출력.pnm}}`

- 결과 이미지가 지정된 너비를 갖도록 이미지 확장하며, 가로세로 비율 유지:

`pnmscalefixed -width {{너비}} {{경로/대상/입력.pnm}} > {{경로/대상/출력.pnm}}`

- 이미지의 너비와 높이를 지정된 배율로 변경하여 확장:

`pnmscalefixed -xscale {{x_배율}} -yscale {{y_배율}} {{경로/대상/입력.pnm}} > {{경로/대상/출력.pnm}}`
