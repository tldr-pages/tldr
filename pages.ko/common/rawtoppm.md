...
`rawtoppm {{너비}} {{높이}} {{경로/대상/이미지.raw}} | pamflip {{[-tb|-topbottom]}} > {{경로/대상/출력.ppm}}`

- 지정된 파일의 처음 n 바이트 무시:

`rawtoppm {{너비}} {{높이}} {{[-h|-headerskip]}} {{n}} {{경로/대상/이미지.raw}} > {{경로/대상/출력.ppm}}`

- 지정된 파일의 각 행에서 마지막 m 바이트 무시:

`rawtoppm {{너비}} {{높이}} {{[-ro|-rowskip]}} {{m}} {{경로/대상/이미지.raw}} > {{경로/대상/출력.ppm}}`
