...
`cat {{경로/대상/파일.py}} | llm {{[-s|--system]}} "{{이 코드를 설명하세요}}"`
...
`llm {{[-m|--model]}} {{orca-mini-3b-gguf2-q4_0}} "{{프랑스의 수도는 어디인가요?}}"`

- [s]시스템 프롬프트를 생성하고 템플릿 이름으로 [s]저장:

`llm {{[-s|--system]}} '{{당신은 지각 있는 치즈케이크입니다}}' --save {{지각있는_치즈케이크}}`

- 특정 [m]모델과 특정 [t]템플릿을 사용하여 대화형 채팅:

`llm chat {{[-m|--model]}} {{chatgpt}} {{[-t|--template]}} {{지각있는_치즈케이크}}`
