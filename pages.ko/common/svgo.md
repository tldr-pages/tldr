...
`svgo {{테스트.svg}} {{[-o|--output]}} {{테스트.min.svg}}`

- 디렉토리 내 모든 SVG 파일 최적화 (원본 파일을 덮어씁니다):

`svgo {{[-f|--folder]}} {{경로/대상/폴더/안의/svg/파일들}}`

- 디렉토리 내 모든 SVG 파일을 최적화하고 결과 파일을 다른 디렉토리에 저장:

`svgo {{[-f|--folder]}} {{입력/디렉토리/경로}} {{[-o|--output]}} {{출력/디렉토리/경로}}`

- 다른 명령에서 전달된 SVG 콘텐츠를 최적화하고 결과를 파일에 저장:

`{{cat 테스트.svg}} | svgo {{[-i|--input]}} - {{[-o|--output]}} {{테스트.min.svg}}`

- 파일을 최적화하고 결과를 출력:

`svgo {{테스트.svg}} {{[-o|--output]}} -`
