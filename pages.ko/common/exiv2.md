# exiv2

> 이미지 메타데이터 도구.
> 더 많은 정보: <https://exiv2.org/manpage.html>.

- 이미지 Exif 메타데이터 요약을 출력:

`exiv2 {{경로/대상/파일}}`

- 해석된 값으로 모든 메타데이터 (Exif, IPTC, XMP)를 출력:

`exiv2 {{[-P|-Print]}} kt {{경로/대상/파일}}`

- 윈시 값으로 모든 메타데이터를 출력:

`exiv2 {{[-P|-Print]}} kv {{경로/대상/파일}}`

- 이미지에서 모든 메타데이터를 삭제:

`exiv2 {{[-d|--delete]}} a {{경로/대상/파일}}`

- 파일 타임스탬프를 유지하면서, 모든 메타데이터를 삭제:

`exiv2 {{[-d|--delete]}} a {{[-k|--keep]}} {{경로/대상/파일}}`

- 메타데이터(파일 타임스탬프가 아님)의 날짜와 시간을 앞에 추가하여 파일 이름을 변경:

`exiv2 {{[-r|--rename]}} '{{%Y%m%d_%H%M%S_:basename:}}' {{경로/대상/파일}}`
