...
`waymore -mode U {{[-oU|--output-urls]}} {{경로/대상/example.com-주소.txt}} {{[-i|--input]}} {{example.com}}`

- URL의 콘텐츠 본문만 출력하고 지정된 디렉토리에 출력 저장:

`waymore -mode R {{[-oR|--output-responses]}} {{경로/대상/example.com-주소-응답}} {{[-i|--input]}} {{example.com}}`
