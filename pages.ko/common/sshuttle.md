...
`sshuttle {{[-r|--remote]}} {{사용자_명}}@{{ssh서버}} {{0.0.0.0/0}}`

- 서버의 기본 DNS 해석기로 모든 DNS 트래픽도 전달:

`sshuttle --dns {{[-r|--remote]}} {{사용자_명}}@{{ssh서버}} {{0.0.0.0/0}}`

- 특정 서브넷으로 향하는 트래픽을 제외한 모든 트래픽 전달:

`sshuttle {{[-r|--remote]}} {{사용자_명}}@{{ssh서버}} {{0.0.0.0/0}} {{[-x|--exclude]}} {{192.168.0.1/24}}`

- tproxy 방법을 사용하여 모든 IPv4 및 IPv6 트래픽 전달:

`sshuttle --method tproxy {{[-r|--remote]}} {{사용자_명}}@{{ssh서버}} {{0.0.0.0/0}} {{::/0}} {{[-x|--exclude]}} {{내_로컬_ip_주소}} {{[-x|--exclude]}} {{ssh_서버_ip_주소}}`
