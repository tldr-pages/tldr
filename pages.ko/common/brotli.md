...
`brotli {{[-d|--decompress]}} {{경로/대상/파일.br}}`

- 출력([o]utput) 파일 이름을 지정하여, 파일을 압축:

`brotli {{경로/대상/파일}} {{[-o|--output]}} {{경로/대상/압축된_출력_파일.br}}`

- 출력([o]utput) 파일 이름을 지정하여 Brotli 파일을 압축 해제([d]ecompress)ㄴ:

`brotli {{[-d|--decompress]}} {{경로/대상/압축된_파일.br}} {{[-o|--output]}} {{경로/대상/출력_파일}}`

- 압축 품질을 지정 (1=가장 빠름 (가장 안 좋은 품질), 11=가장 느림 (가장 좋은 품질)):

`brotli {{[-q|--quality]}} {{11}} {{경로/대상/파일}} {{[-o|--output]}} {{경로/대상/압축된_출력_파일.br}}`
