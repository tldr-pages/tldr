...
`pdfgrep {{[-H|--with-filename]}} {{[-n|--page-number]}} {{패턴}} {{파일.pdf}}`

- "foo"로 시작하는 줄을 대소문자 구분 없이 검색하고 처음 3개의 일치 항목 반환:

`pdfgrep {{[-m|--max-count]}} {{3}} {{[-i|--ignore-case]}} '{{^foo}}' {{파일.pdf}}`

- 현재 디렉토리에서 `.pdf` 확장자를 가진 파일을 재귀적으로 검색하여 패턴 찾기:

`pdfgrep {{[-r|--recursive]}} {{패턴}}`

- 현재 디렉토리에서 특정 글롭과 일치하는 파일을 재귀적으로 검색하여 패턴 찾기:

`pdfgrep {{[-r|--recursive]}} --include '{{*book.pdf}}' {{패턴}}`
