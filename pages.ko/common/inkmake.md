...
`inkmake {{[-v|--verbose]}} {{경로/대상/Inkfile}}`

- SVG 입력 파일과 출력 파일을 지정하여, Inkfile을 실행:

`inkmake {{[-s|--svg]}} {{경로/대상/파일.svg}} {{[-o|--out]}} {{경로/대상/출력_이미지}} {{경로/대상/Inkfile}}`

- 사용자 정의 Inkscape 바이너리를 백엔드로 사용:

`inkmake {{[-i|--inkscape]}} {{/Applications/Inkscape.app/Contents/Resources/bin/inkscape}} {{경로/대상/Inkfile}}`
