...
`pnmtopclxl -dpi {{해상도}} {{[-x|-xoffs]}} {{x_오프셋}} {{[-y|-yoffs]}} {{y_오프셋}} {{경로/대상/입력1.pnm 경로/대상/입력2.pnm ...}} > {{경로/대상/출력.pclxl}}`

- 지정된 용지 형식에 대해 양면 프린터 스트림 생성:

`pnmtopclxl {{[-du|-duplex]}} {{세로|가로}} {{[-fo|-format]}} {{letter|legal|a3|a4|a5|...}} {{경로/대상/입력1.pnm 경로/대상/입력2.pnm ...}} > {{경로/대상/출력.pclxl}}`
