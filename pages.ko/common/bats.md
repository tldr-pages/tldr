...
`bats {{[-t|--tap]}} {{경로/대상/test.bats}}`

- 테스트를 실행하지 않고 테스트 스크립트의 테스트 케이스 수([c]ount)를 계산:

`bats {{[-c|--count]}} {{경로/대상/test.bats}}`

- BATS 테스트 케이스를 반복적으로([r]ecursively) 실행 (`.bats` 확장자를 가진 파일):

`bats {{[-r|--recursive]}} {{경로/대상/디렉터리}}`

- 특정 형식([F]ormat)으로 결과를 출력:

`bats {{[-F|--formatter]}} {{pretty|tap|tap13|junit}} {{경로/대상/test.bats}}`

- 테스트에 타이밍([T]iming) 정보 추가:

`bats {{[-T|--timing]}} {{경로/대상/test.bats}}`

- 특정 개수의 작업([j]obs)을 병렬로 실행 (GNU `parallel` 을 설치해야 함):

`bats {{[-j|--jobs]}} {{number}} {{경로/대상/test.bats}}`
