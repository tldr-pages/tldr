...
`zipgrep {{[-H|--with-filename]}} {{[-n|--line-number]}} "{{검색_패턴}}" {{경로/대상/파일.zip}}`

- 패턴과 일치하지 않는 줄 검색:

`zipgrep {{[-v|--invert-match]}} "{{검색_패턴}}" {{경로/대상/파일.zip}}`
...
`zipgrep "{{검색_패턴}}" {{경로/대상/파일.zip}} {{[-x|--line-regexp]}} {{제외할/파일1}} {{제외할/파일2}}`
