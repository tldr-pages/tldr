...
`pamundice {{파일이름_%1d_%1a.ppm}} {{[-a|-across]}} {{x_값}} {{[-d|-down]}} {{y_값}} {{[-h|-hoverlap]}} {{값}} {{[-v|-voverlap]}} {{값}} > {{경로/대상/출력.ppm}}`

- 결합할 이미지를 한 줄에 하나의 파일 이름이 있는 텍스트 파일로 지정:

`pamundice -listfile {{경로/대상/파일.txt}} {{[-a|-across]}} {{x_값}} {{[-d|-down]}} {{y_값}} > {{경로/대상/출력.ppm}}`
