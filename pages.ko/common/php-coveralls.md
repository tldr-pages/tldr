...
`php-coveralls {{[-r|--root_dir]}} {{경로/대상/폴더}}`

- 특정 설정 파일을 사용하여 Coveralls에 커버리지 정보를 전송:

`php-coveralls {{[-c|--config]}} {{경로/대상/.coveralls.yml}}`
...
`php-coveralls {{[-x|--coverage_clover]}} {{경로/대상/첫번째_clover.xml}} --coverage_clover {{경로/대상/두번째_clover.xml}}`

- Coveralls에 전송될 JSON을 특정 파일로 출력:

`php-coveralls {{[-o|--json_path]}} {{경로/대상/coveralls-전송.json}}`
