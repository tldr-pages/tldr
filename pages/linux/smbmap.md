...
`smbmap {{[-u|--username]}} {{username}} --prompt -d {{domain}} -H {{ip}}`

- Display SMB shares and list a single level of directories and files:

`smbmap {{[-u|--username]}} {{username}} --prompt -H {{ip}} -r`
...
`smbmap {{[-u|--username]}} {{username}} --prompt -H {{ip}} -R -A {{pattern}}`
...
`smbmap {{[-u|--username]}} {{username}} --prompt -H {{ip}} --upload {{source}} {{destination}}`
