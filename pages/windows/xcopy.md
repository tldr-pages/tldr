...
`xcopy {{path/to/input}} {{path/to/destination}}`

- List files that will be copied before copying:

`xcopy {{path/to/input}} {{path/to/destination}} /p`

- Copy the directory structure only, excluding files:

`xcopy {{path/to/input}} {{path/to/destination}} /t`

- Include empty directories when copying:

`xcopy {{path/to/input}} {{path/to/destination}} /e`

- Keep the source ACL in the destination:

`xcopy {{path/to/input}} {{path/to/destination}} /o`

- Allow resuming when network connection is lost:

`xcopy {{path/to/input}} {{path/to/destination}} /z`

- Disable the prompt when the file exists in the destination:

`xcopy {{path/to/input}} {{path/to/destination}} /y`
