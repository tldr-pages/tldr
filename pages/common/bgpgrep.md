# bgpgrep

> filter and print BGP data within MRT dumps
> can read files compressed with gzip, bzip2 and xz.
> More information: <https://gitea.it/1414codeforge/ubgpsuite>.

- Outputs all routes:

`bgpgrep {{master6.mrt}}`

- Outputs all routes received from a specific peer, determined by the AS number:

`bgpgrep {{master4.mrt}} -peer {{64498}}`

- Outputs all routes received from a specific peer, determined by the peer's IP address:

`bgpgrep {{master4.mrt.bz2}} -peer {{2001:db8:dead:cafe:acd::19e}}`

- Outputs all routes which have certain AS in their AS path:

`bgpgrep {{master6.mrt.bz2}} -aspath '{{64498 64510}}'`

- Outputs all routes that lead to a specific address:

`bgpgrep {{master6.mrt.bz2}} -supernet '{{2001:db8:dead:cafe:aef::5}}'`

- Outputs all routes that have communities from a specific AS:

`bgpgrep {{master4.mrt}} -communities \( '{{64497}}:*' \)`
