...
- Enable an application over USB or NFC (`-e` or `--enable` can be used multiple times to specify more applications):

`ykman config {{usb|nfc}} {{[-e|--enable]}} {{otp|u2f|fido2|oath|piv|openpgp|hsmauth}}`

- Disable an application over USB or NFC (`-d` or `--disable` can be used multiple times to specify more applications):
