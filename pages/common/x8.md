...
> A hidden parameters discovery suite written in Rust for identifying vulnerable or interesting web parameters.
...
`x8 {{[-u|--url]}} {{https://example.com/?path=../action.php%3f%s%23}} --encode {{[-w|--wordlist]}} {{path/to/wordlist.txt}}`

- Discover hidden headers for a URL:

`x8 {{[-u|--url]}} {{https://example.com/}} --headers {{[-w|--wordlist]}} {{headers.txt}}`
