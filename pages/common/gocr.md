...
`gocr -m 130 -p {{path/to/existing_db_folder}} -i {{path/to/input_image.png}} -o {{path/to/output_file.txt}} -C "{{0..9}}"`

- Recognize characters with a cert[a]inty of 100% (characters have a higher chance to be treated as unknown):

`gocr -m 130 -p {{path/to/existing_db_folder}} -i {{path/to/input_image.png}} -o {{path/to/output_file.txt}} -a 100`
