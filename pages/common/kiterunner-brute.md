...
`kiterunner brute {{https://target.com}} {{[-w|--wordlist]}} {{wordlist.txt}}`

- Bruteforce using a dirsearch-style wordlist with extension substitution:

`kiterunner brute {{https://target.com}} {{[-w|--wordlist]}} {{dirsearch.txt}} {{[-D|--dirsearch-compat]}} {{[-e|--extensions]}} {{json,txt}}`

- Bruteforce with specific file extensions appended and output in JSON format:

`kiterunner brute {{https://target.com}} {{[-w|--wordlist]}} {{wordlist.txt}} {{[-e|--extensions]}} {{aspx,ashx}} {{[-o|--output]}} {{json}}`

- Bruteforce a list of targets from a file with custom concurrency settings for performance:

`kiterunner brute {{targets.txt}} {{[-w|--wordlist]}} {{wordlist.txt}} {{[-x|--max-connection-per-host]}} {{5}} {{[-j|--max-parallel-hosts]}} {{100}}`

- Bruteforce and ignore specific content length responses:

`kiterunner brute {{https://target.com}} {{[-w|--wordlist]}} {{wordlist.txt}} --ignore-length={{100-105}}`

- Bruteforce with custom HTTP headers:

`kiterunner brute {{https://target.com}} {{[-w|--wordlist]}} {{wordlist.txt}} {{[-H|--header]}} "{{Authorization: Bearer token}}"`

- Bruteforce a list of targets from a file with fail status code filtering:

`kiterunner brute {{targets.txt}} {{[-w|--wordlist]}} {{wordlist.txt}} --fail-status-codes {{400,401,404}}`
