...
`watch {{class_pattern}} {{method_pattern}} '{{{ params[0],returnObj }}}' -x 4`
...
`watch {{class_pattern}} {{method_pattern}} '{{{ params[1],returnObj }}}' '{{"5".equals(params[0])}}' -x 4`

- When the method returns or an exception occurs, observe the count property of the second parameter:

`watch {{class_pattern}} {{method_pattern}} '{{{ params[1].count }}}' -e -s`
