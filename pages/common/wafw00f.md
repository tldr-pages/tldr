...
`wafw00f --proxy {{http://localhost:8080}} {{https://example.com}}`

- [t]est for a specific WAF product (run `wafw00f -l` to get list of all supported WAFs):

`wafw00f --test {{Cloudflare|Cloudfront|Fastly|ZScaler|...}} {{https://example.com}}`

- Pass custom [H]eaders from a file:

`wafw00f --headers {{path/to/headers.txt}} {{https://example.com}}`
