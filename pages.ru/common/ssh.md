...
- Запустить команду на удаленном сервере с выделением tty [t], что позволяет взаимодействовать с удаленной командой:
...
- SSH-туннелирование: переадресовать определенный порт (`localhost:9999` на `example.com:80`) с отключением псевдо-tty [T] и выполне[N]ия удаленных команд:
