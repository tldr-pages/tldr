...
`ssh {{имя_пользователя}}@{{удаленный_хост}} -i {{путь/к/файлу_ключа}}`
...
`ssh {{имя_пользователя}}@{{удаленный_хост}} -D {{1080}}`
...
`ssh {{имя_пользователя}}@{{удаленный_хост}} -L {{9999}}:{{example.com}}:{{80}} -N -T`

- SSH-прыжок [J]: подключиться к удаленному серверу через промежуточный хост (можно указать несколько хостов через запятую):

`ssh {{имя_пользователя}}@{{удаленный_хост}} -J {{имя_пользователя}}@{{промежуточный_хост}}`
