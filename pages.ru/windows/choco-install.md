...
- Принудительно переустановить пакет:

`choco install {{пакет}} {{[-f|--force]}}`

- Автоматически подтвердить все запросы:

`choco install {{пакет}} {{[-y|--yes]}}`

- Указать пользовательский источник для получения пакетов:

`choco install {{пакет}} {{[-s|--source]}} {{url_источника|псевдоним}}`

- Указать имя пользователя и пароль для аутентификации:

`choco install {{пакет}} {{[-u|--user]}} {{имя_пользователя}} {{[-p|--password]}} {{пароль}}`
