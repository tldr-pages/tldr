...
`pg_restore {{[-d|--dbname]}} {{nombre_base_de_datos}} {{archivo_de_datos.dump}}`

- Igual que antes, utilizando un nombre de usuario:

`pg_restore {{[-U|--username]}} {{usuario}} {{[-d|--dbname]}} {{nombre_base_de_datos}} {{archivo_de_datos.dump}}`

- Lo mismo que antes, usando un nombre de equipo y puerto:

`pg_restore {{[-h|--host]}} {{equipo}} {{[-p|--port]}} {{puerto}} {{[-d|--dbname]}} {{nombre_base_de_datos}} {{archivo_de_datos.dump}}`

- Lista los objetos de bases de datos incluidos en el archivo:

`pg_restore {{[-l|--list]}} {{archivo_de_datos.dump}}`

- Limpia los objetos de base de datos antes de crearlos:

`pg_restore {{[-c|--clean]}} {{[-d|--dbname]}} {{nombre_base_de_datos}} {{archivo_de_datos.dump}}`

- Utiliza múltiples trabajos para hacer la restauración:

`pg_restore {{[-j|--jobs]}} {{2}} {{[-d|--dbname]}} {{nombre_base_de_datos}} {{archivo_de_datos.dump}}`
