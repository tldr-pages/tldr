...
exports.update = function(opts) {
  var dirs = typeof opts.update === 'string' ? opts.update.split(',') : 'all';
  request.getPaths(opts, function(err, paths) {
    if (err) {
      console.log('ERR: ' + err);
    } else {
      paths.forEach(function(path) {
        if (dirs.indexOf(path.name) === -1 && dirs.indexOf('all') === -1) {
          return;
        }
        opts.name = path.name;
        request.getFiles(opts, function(err, files) { // im assuming the structure is proper.... hrm....
          if (err) {
            console.log('ERR: ' + err);
          } else {
            files.forEach(function(file) {
              console.log("Downloading " + path.name + '/' + file.name);
              request.get(path.name + "/" + file.name, function(err, contents) {
                if (err) {
                  console.log('ERR: ' + err);
                }
              });
