...
`docker exec {{[-it|--interactive --tty]}} --workdir {{chemin/vers/le/répertoire}} {{nom_du_conteneur}} {{commande}}`
...
`docker exec {{[-it|--interactive --tty]}} --env {{variable_d_environnement}}={{valeur}} {{nom_du_conteneur}} {{/bin/bash}}`
