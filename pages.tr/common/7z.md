...
`7z a {{sikistirilmis-dosya.7z}} {{yoldan/dosya_veya_dizine}}`

- Varolan bir arşivi çözümle (headerlar dahil):

`7z a {{sifrelenmiş-dosya.7z}} -p{{parola}} -mhe=on {{archived.7z}}`

- Varolan 7z dosyasını orijinal dizin yapısıyla dışa aktar:

`7z x {{sikistirilmis-dosya.7z}}`

- Arşivi kullanıcı tarafından belirtilmiş çıkış noktasına aktar:

`7z x {{sikistirilmis-dosya.7z}} -o{{yoldan/çıktıya}}`

- Arşivi stdout'a aktar:

`7z x {{sikistirilmis-dosya.7z}} -so`
