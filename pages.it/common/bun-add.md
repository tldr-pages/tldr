...
`bun {{[a|add]}} {{pacchetto1 pacchetto2 ...}}`
...
`bun {{[a|add]}} {{pacchetto}}@{{versione}}`

- Installa da un file o una directory locale:

`bun {{[a|add]}} file:{{percorso/del/file_o_directory}}`

- Aggiungi una dipendenza di sviluppo:

`bun {{[a|add]}} {{[-d|--dev]}} {{pacchetto}}`

- Aggiungi un pacchetto globalmente:

`bun {{[a|add]}} {{[-g|--global]}} {{pacchetto}}`
