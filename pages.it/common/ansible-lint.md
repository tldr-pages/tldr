...
`ansible-lint {{percorso/al/file_playbook}} {{percorso/alla/role_directory}}`

- Lint un playbook escludendo regole specifiche:

`ansible-lint {{[-x|--exclude-rules]}} {{regola1,regola2,...}} {{percorso/al/file_playbook}}`

- Lint un playbook in modalità offline e formatta l'output come PEP8:

`ansible-lint {{[-o|--offline]}} {{[-p|--parseable]}} {{percorso/al/file_playbook}}`

- Lint un playbook utilizzando una directory di regole personalizzate:

`ansible-lint {{[-r|--rules]}} {{percorso/alla/custom_rules_directory}} {{percorso/al/file_playbook}}`

- Lint ricorsivamente tutto il contenuto Ansible in una directory specificata:

`ansible-lint {{percorso/alla/project_directory}}`
